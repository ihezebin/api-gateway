...
func RuleMatcher(endpoints []*entity.Endpoint, rules []*entity.Rule) (gin.HandlerFunc, error) {
	...
	return func(c *gin.Context) {
		...
		if rule == nil {
			...
			body.WithErr(httpserver.NewError(httpserver.CodeNotFound, fmt.Sprintf("%s + %s 未注册域名到网关", domain, path)))
		...
		if uri == nil {
			...
			body := &httpserver.Body[any]{}
			body.WithErr(httpserver.NewError(httpserver.CodeNotFound, fmt.Sprintf("%s + %s 未注册路由到网关", domain, path)))
			c.AbortWithStatusJSON(http.StatusNotFound, body)
			return
		}
